    ...
    return `
        <tr class="hover:bg-gray-50 transition-colors duration-150">
            ...
            <td class="px-6 py-4 text-sm font-normal text-slate-500 tabular-nums">
               ${statusLabels(record.actually)}
            </td>
             <td class="px-6 py-4 text-sm font-normal text-slate-500 tabular-nums capitalize">
                ${statusLabels(record.open_registration)}
            </td>
            <td class="px-5 py-3 text-xs">
                <div class="flex justify-center gap-2">
                    <button class="btn-edit p-2 text-blue-500 bg-blue-50 hover:bg-blue-100 border rounded-xl transition-all" 
                            data-id="${record.codperiod}" 
                            title="Editar">
...
function statusLabels(status) {
    const statusMap = {
        Y: '<span class="px-2 inline-flex text-[10px] leading-5 font-semibold rounded-full bg-green-100 text-green-800 uppercase">Activado</span>',
        N: '<span class="px-2 inline-flex text-[10px] leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800 uppercase">Desactivado</span>',
    };
    return (
        statusMap[status] ||
        '<span class="px-2 inline-flex text-[10px] leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Desconocido</span>'
    );
}
