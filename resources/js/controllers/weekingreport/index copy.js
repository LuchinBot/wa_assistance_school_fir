...
export default function () {
    ...
    let currentTab = "summary"; // 'summary' | 'detail'
    ...
    /* =============================================
       TABLE ENGINE — RESUMEN
    ============================================= */
    const tableSummary = new TableEngine({
        ...
        onLoad(data) {
            // Actualizar contador y label de días
            const totalDaysLabel = document.getElementById("totalDaysLabel");
            if (totalDaysLabel && data.total_days !== undefined) {
                totalDaysLabel.textContent =
                    data.total_days > 0
                        ? `· ${data.total_days} día${data.total_days !== 1 ? "s" : ""} en el rango`
                        : "";
            }
        },
        ...
        createRow(record, index) {
            ...
            const rowNum =
                (tableSummary.state?.currentPage - 1) *
                    tableSummary.config.recordsPerPage +
                index +
                1;
            ...
            return `
                <tr style="border-bottom: 1px solid #f1f5f9; transition: background 0.1s;"
                    ...
                    <td class="px-5 py-3 text-xs text-slate-400 font-semibold">${rowNum}</td>

                    <td class="px-5 py-3">
                        <p class="text-xs font-semibold text-slate-800">
                            ${person.firstname ?? "—"} ${fullName}
                        </p>
                        <p class="text-[11px] text-slate-400 mt-0.5">DNI: ${person.identify_number ?? "—"}</p>
                    </td>
    ...
    /* =============================================
       TABLE ENGINE — DETALLE
    ============================================= */
    const tableDetail = new TableEngine({
        baseUrl: () => "/report/weeking",
        search: true,
        pagination: true,
        loading: true,
        tableBodyId: "tableBodyDetail",
        cardContainerId: "cardContainerDetail",
        ...
        createRow(record) {
            ...
            const sessionDate = record.assistance_session?.date
                ? new Date(record.assistance_session.date).toLocaleDateString(
                      "es-PE",
                      {
                          day: "2-digit",
                          month: "2-digit",
                ...
                <div class="p-4 bg-white border-b border-slate-100">
                    <div class="flex items-center justify-between gap-2">
                        <div>
                            ...
                            <p class="text-xs text-slate-400 mt-0.5">DNI: ${person.identify_number ?? "—"} · ${sessionDate}</p>
                        </div>
    ...
    function reload() {
        if (currentTab === "summary") {
            tableSummary.state.currentPage = 1;
            tableSummary.loadRecords(0, tableSummary.config.recordsPerPage);
        } else {
            tableDetail.state.currentPage = 1;
            tableDetail.loadRecords(0, tableDetail.config.recordsPerPage);
        }
    }
    ...
    btnClearFilters?.addEventListener("click", () => {
        ...
        // Restaurar opciones de grados originales
        document
            .querySelectorAll("#gradeFilter option[data-original]")
            .forEach((o) => o.remove());
        sectionFilter.innerHTML =
            '<option value="">Todas las secciones</option>';
        sectionFilterWrap.classList.add("att-hidden");
