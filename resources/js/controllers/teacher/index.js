...
function createTableRow(record) {
    const fullName = `${record.person.lastname_father || ""} ${
        record.person.lastname_mom || ""
    }`.trim();

    const logoPath =
        record.person && record.person.photo_url
            ? record.person.photo_url
            : "/img/person.jpg";
    ...
    return `
        <tr class="hover:bg-gray-50 transition-colors duration-150">
            ...
            <td class="px-6 py-4 text-sm font-normal text-slate-500 tabular-nums">
                ${record.profession.name_large || "-"}
            </td>
            <td class="px-6 py-4 text-sm font-semibold text-black tabular-nums">
