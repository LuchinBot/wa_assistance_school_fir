...
function loadInitialRecords() {
    const initialData = countRecords;
    if (initialData > 0) {
...
function setupEventListeners() {
    ...
    $(document).on("click", ".btn-download", function () {
        const id = $(this).data("media");
        window.location.href = `${CONFIG.baseUrl}/voucher/download/${id}`;
    });
}
 ...
 */
function loadRecords(from, to) {
    if (STATE.isLoading) return;

    STATE.isLoading = true;
    showLoading();

    const keywordParam = STATE.searchKeyword || "null";

    $.ajax({
        url: `${CONFIG.baseUrl}/records/${from}/${to}/${keywordParam}`,
        method: "GET",
        success: function (response) {
            if (response.success) {
                totalRecords = response.total;
...
/**
 * Crear fila de tabla con soporte para PDF
 */
function createTableRow(record) {
    const fullName = `${record.journalist.person.firstname || ""} ${
        record.journalist.person.lastname_father || ""
    } ${record.journalist.person.lastname_mom || ""}`.trim();

    // Determinar si es PDF o imagen
    let filePreview;
    if (record.photo) {
        const extension = record.photo.split(".").pop().toLowerCase();
        const isPdf = extension === "pdf";

        if (isPdf) {
            filePreview = `
                <div class="w-10 h-10 flex items-center justify-center bg-blue-50 rounded cursor-pointer preview-pdf-main" data-url="/storage/${record.photo}">
            ...
            //filePreview = `<div class="w-10 h-10 border-gray-300 bg-gray-500"><img src="/storage/${record.photo}" alt="Foto de voucher" class="w-full h-full object-cover cursor-pointer preview-image-main opacity-50 hover:opacity-100 transition-opacity duration-300"/></div>`;
            filePreview = `<div class="relative w-12 h-12 group/img">
                    <img src="/storage/${record.photo}" 
                        class="w-full h-full object-cover rounded-md shadow-sm border-2 border-white group-hover/img:scale-110 transition-transform cursor-pointer preview-image-main shadow-slate-200"
                        alt="Foto">
                    <div class="absolute inset-0 rounded-2xl bg-black/5 opacity-0 group-hover/img:opacity-100 transition-opacity pointer-events-none"></div>
                </div>`;
        }
    } else {
        filePreview = `<div class="w-10 h-10 border-gray-300 bg-gray-500">
                        <img src="/img/voucher/default.png" alt="Sin archivo" class="w-full h-full object-cover opacity-50 hover:opacity-100 transition-opacity duration-300"/>
                        </div>`;
    }
        ...
        <tr class="hover:bg-gray-50 transition-colors duration-150">
            ...
            <td class="px-5 py-3 text-xs">
              <p class="text-sm font-bold text-slate-800 tracking-tight leading-none">
                    ${record.journalist.person.firstname} ${
                        record.journalist.person.lastname_father
                    }
                </p>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mt-1">
                    ${record.journalist.person.identify_number || ""}
                </p>
            </td>
            <td class="px-4 py-4 text-sm font-normal text-slate-800 tabular-nums">
                ${record.concepto || "-"}
            </td>
            <td class="px-6 py-4 text-sm font-normal text-black tabular-nums">
                S/ ${parseFloat(record.monto).toFixed(2) || "-"}
            </td>
            <td class="px-6 py-4 text-sm font-normal text-slate-500 tabular-nums">
                ${record.datetime || "-"}
            </td>
            <td class="px-5 py-3 text-xs">
                <div class="flex justify-center gap-1">
                    <button class="btn-download p-2 text-emerald-500 hover:bg-emerald-50 rounded-xl transition-all" 
                    data-media="${record.codvoucher}" 
                    title="Descargar">
...
// PARA VER LAS IMÁGENES (event delegation)
$(document).on("click", ".preview-image-main", function () {
    const src = $(this).attr("src");
    $("#modal-image-main .modal-body img").attr("src", src);
    $("#modal-image-main").fadeIn();
});

// PARA ABRIR PDFs en nueva pestaña (event delegation)
$(document).on("click", ".preview-pdf-main", function () {
...
// PARA VER LAS FOTOS (event delegation)
$(document).on("click", ".preview-image-main", function () {
    const src = $(this).attr("src");
    $("#modal-image-main .modal-body img").attr("src", src);
    $("#modal-image-main").fadeIn();
