 ...
 */
function validateIdentifyNumber(number) {
    // Aceptar números y letras (para pasaportes)
...
function validatePhoto(file) {
    ...
    const isPdf = file.type === CONFIG.allowedPdfType;

    // Validar tipo
    if (!isImage && !isPdf) {
        showError("Solo se permiten imágenes JPG, PNG o archivos PDF");
        photoInput.val("");
        return false;
    }
...
/**
 * Manejar preview de foto o PDF
 */
/**
 * Manejar preview de foto o PDF con opción de eliminar
 */
function handlePhotoPreview(input) {
    if (input.files && input.files[0]) {
        ...
        const isPdf = file.type === CONFIG.allowedPdfType;
        $("#fileType").val(isPdf ? "pdf" : "image");

        // Contenedor base para la vista previa con botón de eliminar
        const wrapperStart = `<div class="relative w-full h-full group">
            <button type="button" onclick="removeSelectedFile()" class="absolute top-2 right-2 bg-blue-500 text-white rounded-full p-1 shadow-lg hover:bg-blue-600 z-10">
        ...
        if (isPdf) {
            photoPreview.html(
                ...
                <div class="w-full h-full flex flex-col items-center justify-center p-4">
                    <span class="material-symbols-outlined text-slate-300 text-5xl mb-2">cloud_upload</span>
                    <span class="text-xs text-gray-600 mt-2 font-medium">PDF</span>
                    <span class="text-xs text-gray-500 mt-1 truncate w-full text-center">${file.name}</span>
                </div>` +
                    wrapperEnd,
            );
        } else {
            ...
            reader.onload = function (e) {
                photoPreview.html(
                    wrapperStart +
                        `
                    <img src="${e.target.result}" alt="Preview" class="w-full h-full object-cover rounded">` +
                        wrapperEnd,
                );
            };
...
/**
 * Eliminar el archivo seleccionado o del servidor
 */
function removeSelectedFile() {
    const recordId = $("#recordId").val();

    // Si es un archivo nuevo que aún no se guarda, solo limpiamos el input y la vista
    if (!recordId || photoInput[0].files.length > 0) {
        resetPhotoPreview();
        return;
    }

    // Si el registro ya existe, preguntamos antes de eliminar del servidor
    if (
        confirm(
            "¿Estás seguro de que deseas eliminar permanentemente este archivo del servidor?",
        )
    ) {
        $.ajax({
            url: `${CONFIG.baseUrl}/delete-file/${recordId}`,
            method: "DELETE",
            dataType: "json",
            beforeSend: function () {
                showWaiting("Eliminando archivo...");
            },
            success: function (response) {
                if (response.success) {
                    resetPhotoPreview();
...
/**
 * Restablece el componente de foto a su estado inicial
 */
