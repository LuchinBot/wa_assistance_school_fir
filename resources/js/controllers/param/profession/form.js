...
function loadProvinces(coddepartment) {
    if (!coddepartment) {
        provinceSelect
            ...
            .prop("disabled", true);
        districtSelect
            .html('<option value="">Seleccione provincia primero...</option>')
            .prop("disabled", true);
        codubigeoInput.val("");
        return;
    }
    ...
    $.ajax({
        url: `/${controller}/provinces/${coddepartment}`,
        method: "GET",
        success: function (response) {
            if (response.success) {
                let options = '<option value="">Seleccione...</option>';
                response.data.forEach(function (province) {
                    options += `<option value="${province.codprovince}">${province.name}</option>`;
                });
        ...
        error: function (xhr) {
            provinceSelect.html('<option value="">Error al cargar</option>');
            handleAjaxError(xhr);
        },
    });
}
...
function loadDistricts(coddepartment, codprovince) {
    ...
    $.ajax({
        url: `/${controller}/districts/${coddepartment}/${codprovince}`,
        method: "GET",
        success: function (response) {
            if (response.success) {
                let options = '<option value="">Seleccione...</option>';
                response.data.forEach(function (district) {
                    options += `<option value="${district.coddistrict}" data-ubigeo="${district.codubigeo}">${district.name}</option>`;
                });
                ...
                showError("Error al cargar los distritos");
            }
        },
        error: function (xhr) {
            districtSelect.html('<option value="">Error al cargar</option>');
            handleAjaxError(xhr);
        },
    });
}
