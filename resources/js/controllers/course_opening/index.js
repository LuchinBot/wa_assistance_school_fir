...
function createTableRow(record) {
    const course = record.course.name_large || "-";
    const group = record.group.name_large || "-";
    const modality = record.modality.name_large || "-";
    const classroom = record.classroom.name_large || "-";
    const filial = record.filial.name_large || "-";

    const teacher = record.teacher.person
        ? `${record.teacher.person.firstname} ${record.teacher.person.lastname_father}`
        : "-";

    const webInscription = record.web_inscription
        ? `<span class="px-2 py-1 text-[10px] font-bold bg-green-50 text-green-600 rounded-lg">ACTIVO</span>`
        : `<span class="px-2 py-1 text-[10px] font-bold bg-red-50 text-red-500 rounded-lg">INACTIVO</span>`;

    const imagePath = record.web_image
        ? `/storage/${record.web_image}`
        : "/img/no-image.jpg";
    ...
    return `
        <tr class="hover:bg-gray-50 transition-colors duration-150">
            ...
            <td class="px-6 py-4 text-sm">
                <span class="px-2 py-1 bg-blue-50 text-blue-600 rounded-lg text-[11px] font-bold">
                    ${modality}
                </span>
            </td>
