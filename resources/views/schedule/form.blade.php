...
@section('content')
    <div class="min-h-screen" style="background: #f4f6f8;">
        ...
        <div class="bg-white px-6 lg:px-10 py-4 flex items-center gap-4" style="border-bottom: 1px solid #e8edf2;">
            ...
            <span class="ml-1 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider"
                ...
                {{ isset($schedules) ? 'Editando' : 'Nuevo' }}
            </span>
        </div>
        ...
        <div class="px-6 lg:px-10 py-6 max-w-3xl mx-auto">
            ...
            <form id="mainForm" enctype="multipart/form-data" novalidate>
                @csrf
                <input type="hidden" id="recordId" value="{{ $schedules->codschedule ?? '' }}">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                            ...
                            <div class="space-y-1.5">
                                <label for="turn" class="flex items-center gap-1 text-xs text-slate-500">
                                    Turno <span class="text-red-400">*</span>
                                </label>
                                <input type="text" id="turn" name="turn" value="{{ $schedules->turn ?? '' }}"
                                ...
                                <span class="error-message hidden text-[11px] font-medium text-red-500"
                                    data-error-for="turn"></span>
                            </div>

                            {{-- Hora de inicio --}}
                            <div class="space-y-1.5">
                                <label for="time_start" class="flex items-center gap-1 text-xs text-slate-500">
                                    Hora de inicio <span class="text-red-400">*</span>
                                </label>
                                <input type="time" id="time_start" name="time_start"
                                    ...
                                    onblur="this.style.background='#f8fafc'; this.style.borderColor='#e2e8f0'; this.style.boxShadow='none';">
                                <span class="error-message hidden text-[11px] font-medium text-red-500"
                                    data-error-for="time_start"></span>
                            </div>

                            {{-- Hora de fin --}}
                            <div class="space-y-1.5">
                                <label for="time_end" class="flex items-center gap-1 text-xs text-slate-500">
                                    Hora de fin <span class="text-red-400">*</span>
                                </label>
                        ...
                        <div class="flex items-center gap-2 w-full sm:w-auto">
                            ...
                            <button type="submit" id="btnSubmit"
                                ...
                                <span id="btnSubmitText">
                                    {{ isset($schedules) ? 'Actualizar' : 'Guardar' }}
                                </span>
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        const recordId = "{{ $schedules->codschedule ?? '' }}";
