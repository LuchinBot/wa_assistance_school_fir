...
@section('content')
    <div class="min-h-screen" style="background: #f4f6f8;">
        ...
        <div class="bg-white px-6 lg:px-10 py-4 flex items-center gap-4" style="border-bottom: 1px solid #e8edf2;">
            ...
            <span class="ml-1 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider"
                ...
                {{ isset($permission) ? 'Editando' : 'Nuevo' }}
            </span>
        </div>
        ...
        <div class="px-6 lg:px-10 py-6 max-w-3xl mx-auto">
            ...
            <form id="mainForm" enctype="multipart/form-data" novalidate>
                @csrf
                <input type="hidden" id="recordId" value="{{ $permission->codpermission ?? '' }}">
                    ...
                    <div class="p-6 space-y-2">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div class="space-y-1.5">
                                    ...
                                    @if (!isset($permissions))
                                        <div class="ml-2">
                                            <label class="flex items-center gap-1 text-xs text-slate-500">
                                                <input type="checkbox" id="create_default_permissions"
                                                    name="create_default_permissions" value="1"
                                ...
                                <select id="codmodule" name="codmodule"
                                    ...
                                    @foreach ($modules as $module)
                                        <option value="{{ $module->codmodule }}"
                                            {{ isset($permissions) && $permissions->codmodule == $module->codmodule ? 'selected' : '' }}>
                                            {{ $module->name_large }}
                                        </option>
                                    @endforeach
                                </select>
                                <span class="error-message text-red-800 text-xs font-normal mt-1 hidden"
                                    data-error-for="codmodule"></span>
                            </div>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <input type="text" id="name" name="name" value="{{ $permissions->name ?? '' }}"
                        ...
                        <div class="grid grid-cols-1 gap-5">
                            <!-- Abreviatura -->
                            <div class="space-y-1.5">
                                <label for="description" class="flex items-center gap-1 text-xs text-slate-500">
                                    Descripción
                                </label>
                                <input type="text" name="description" id="description"
                                    ...
                                    onblur="this.style.background='#f8fafc'; this.style.borderColor='#e2e8f0'; this.style.boxShadow='none';">
                                <span class="error-message text-red-800 text-xs font-normal mt-1 hidden"
                                    data-error-for="description"></span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        const recordId = "{{ $permission->codpermission ?? '' }}";
