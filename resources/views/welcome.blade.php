...
@section('content')
    ...
    <div class="px-6 lg:px-10 py-8">
        ...
        {{-- ── HEADER ── --}}
        <div class="mb-5">
            <h1 class="text-xl md:text-2xl font-bold text-gray-900 leading-none">
                Bienvenido, {{ Auth::user()->person->firstname }}
            </h1>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                ...
                @if ($authorizedChildren->count() > 0 && $first)
                    <a href="{{ route($first->route) }}"
                        ...
                        <div class="flex items-start justify-between">
                            ...
                            <span class="material-symbols-outlined text-md" style="color: #e2e8f0;">arrow_outward</span>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-400 leading-relaxed">
                                {{ $authorizedChildren->count() }}
                                submódulo{{ $authorizedChildren->count() != 1 ? 's' : '' }}
                                disponible{{ $authorizedChildren->count() != 1 ? 's' : '' }}
                            </p>
                        </div>

                        <div class="flex items-center gap-1 mt-auto" style="color: rgb(0,176,202);">
                            <span class="text-sm md:text-xs font-semibold">Abrir módulo</span>
                            <span
            ...
            <a href="{{ route('user.password') }}"
                ...
                <div class="flex items-start justify-between">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                        style="background: rgba(0,176,202,0.08); border: 1px solid rgba(0,176,202,0.12);">
                        <span class="material-symbols-outlined text-[20px]" style="color: rgb(0,176,202);">lock</span>
                    </div>
                ...
                <div class="flex items-center gap-1 mt-auto" style="color: rgb(0,176,202);">
                    <span class="text-sm md:text-xs font-semibold">Cambiar ahora</span>
