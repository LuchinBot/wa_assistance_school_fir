...
@section('content')
    <div class="min-h-screen" style="background: #f4f6f8;">
        ...
        <div class="bg-white px-6 lg:px-10 py-4 flex items-center gap-4" style="border-bottom: 1px solid #e8edf2;">
            ...
            <span class="ml-1 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider"
                ...
                {{ isset($user_schedule) ? 'Editando' : 'Nuevo' }}
            </span>
        </div>
        ...
        <div class="px-6 lg:px-10 py-6 max-w-4xl mx-auto">
            ...
            <form id="mainForm" enctype="multipart/form-data" novalidate>
                @csrf
                <input type="hidden" id="recordId" value="{{ $user_schedule->coduser_schedule ?? '' }}">
                                ...
                                <select id="codschedule" name="codschedule"
                                    ...
                                    @foreach ($schedules as $schedule)
                                        <option value="{{ $schedule->codschedule }}"
                                            {{ isset($user_schedule) && $user_schedule->codschedule == $schedule->codschedule ? 'selected' : '' }}>
                                            {{ $schedule->turn }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="space-y-1.5">
                                <label for="coduser" class="flex items-center gap-1 text-xs text-slate-500">
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->coduser }}"
                                            {{ isset($user_schedule) && $user_schedule->coduser == $user->coduser ? 'selected' : '' }}>
                                            {{ $user->username }} - {{ $user->person->firstname }} {{ $user->person->lastname_father }}
                                        </option>
                                    @endforeach
                                </select>
                                <span class="error-message hidden text-[11px] font-medium text-red-500"
                                    data-error-for="coduser"></span>
                            </div>
                        ...
                        <div class="flex items-center gap-2 w-full sm:w-auto">
                            ...
                            <button type="submit" id="btnSubmit"
                                ...
                                <span id="btnSubmitText">
                                    {{ isset($user_schedule) ? 'Actualizar' : 'Guardar ' }}
                                </span>
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        const recordId = "{{ $user_schedule->coduser_schedule ?? '' }}";
